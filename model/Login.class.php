	...
	Class Login extends Conexao{
		...
		function GetLogin($user, $senha){
			...
			if($this->TotalDados() > 0){
				...
				#Rotas::Redirecionar(1.5, Rotas::pag_ClienteLogin());
				Rotas::Redirecionar(0.5, Rotas::pag_MinhaConta());
			}else{
				echo '<script> alert("Os dados E-mail ou Senha devem estar incorretos!");</script>';
			}
		}
        ...
        $query = "SELECT * FROM {$this->prefix}user WHERE user_nome = :usuario AND user_pw = :senha";
        
        $params = array(':usuario'=>  $this->getUser(),
                        ':senha'=>  $this->getSenha());
           ...
           if($this->TotalDados() > 0):
             ...
             $_SESSION['ADM']['user_data']   = Sistema::DataAtualBR2();
             $_SESSION['ADM']['user_hora']   = Sistema::HoraAtual();

             return TRUE;
           // caso o login seja incorreto 
           else:
                echo '<h4 class="alert alert-danger"> O login é incorreto </h4>';  

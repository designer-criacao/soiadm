...
if(isset($_POST['btn_gravar']) and isset($_POST['cli_nome']) and isset($_POST['cli_email']) and isset($_POST['cli_cpf'])){
	//Variáveis
...
$clientes->Preparar($cli_nome,$cli_sobrenome,$cli_data_nasc,$cli_rg,$cli_cpf,$cli_ddd,$cli_fone,$cli_celular, $cli_endereco,$cli_numero,$cli_bairro,$cli_cidade,$cli_uf,$cli_cep,$cli_email/*,$cli_data_cad,$cli_hora_cad,$cli_senha*/);
    ...
    if(!$clientes->Inserir()){
        ...
        Rotas::Redirecionar(5, Rotas::get_SiteADM());
        echo '</div>';
        exit();
    }else{
   ...
   // echo '<div class="alert alert-success"> Cadastro Efetuado com Sucesso! </div>';
    //echo '<div class="alert alert-success"> A senha para fazer login foi enviada para seu e-mail cadastrado. <br>'.'Acesse seu e-mail e confira! </div>';
    //Rotas::Redirecionar(1, Rota::pag_ListarClientes());

}//else{}

    $smarty->display('cad_clientes.tpl');
